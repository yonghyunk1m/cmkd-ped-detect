...
Usage:
    # Baseline on ASPED v.a
    python inference.py \
        --checkpoint work_dir/baseline_binary_v1/Session_5242023/best-*.ckpt \
        --config configs/baseline_binary_v1.yaml \
        --data_dir /path/to/ASPED_v.a \
        --output_dir results/predictions_baseline

    # A KD student (LogitKD + TFD)
    python inference.py \
        --checkpoint work_dir/kd_logit_unw_tfd/Session_5242023/best-*.ckpt \
        --config configs/kd_logit_unw_tfd.yaml \
        --data_dir /path/to/ASPED_v.a \
        --output_dir results/predictions_logitkd_tfd \
        --is_kd

    # Run all LOSO folds at once (uses best checkpoint per fold)
    python inference.py \
        --checkpoint_dir work_dir/kd_logit_unw_tfd \
        --config configs/kd_logit_unw_tfd.yaml \
        --data_dir /path/to/ASPED_v.a \
        --output_dir results/predictions_logitkd_tfd
"""

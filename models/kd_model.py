...
class ASPEDKDModel(ASPEDLightningModel):
    ...
    def __init__(self, kd_cfg: dict, **student_kwargs):
        ...
        self.tfd_mask     = kd_cfg.get('tfd_mask', False)
        self.tfd_threshold     = kd_cfg.get('tfd_threshold', 0.4)
        ...
        if self.tfd_mask:
            extras.append(f'TFD-Label(>{self.tfd_threshold})')
    ...
    def _kd_step(self, batch):
        ...
        #   TFD-Label (`tfd_mask`): on minority, teacher agrees with GT
        ...
        if self.tfd_mask:
            ...
            with torch.no_grad():
                ...
                gt_flat = labels.reshape(-1).clamp(min=0)
                suppress = (gt_flat >= 1) & (t_probs[:, 0] > self.tfd_threshold)
        ...
        if self.use_logit_kd and kd_mask.any():
            ...
            if self.therapeutic_alpha:
                ...
                # labels are raw pedestrian counts; clamp to the class-weight range
                # (binary: any count >= 1 maps to the Ped weight)
                idx = labels.reshape(-1)[kd_mask].clamp(max=self.class_weights.numel() - 1)
                cw = self.class_weights[idx]
